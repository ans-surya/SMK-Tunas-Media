...
function isIsolatedRemix(): boolean {
  // Demo build is local-first by default so it never reads/writes the source app database.
  return import.meta.env.VITE_DEMO_MODE !== "false" || (isRemixInstance() && dbId === PRIMARY_DATABASE_ID);
}
